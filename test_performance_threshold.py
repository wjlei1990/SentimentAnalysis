...
def get_train_data(perc, freq):
    ...
    train_data = load_train_data(
        "./data/train.txt", word_count_threshold=5, freq=freq,
        feature_selection_flag=True, percentile=perc)
    return train_data


def get_test_data(vocab, freq):
    test_data = load_test_data("./data/test.txt", vocab, freq=freq)
    return test_data


def accuracy_from_conf(conf):
...
def test_mnbc(train_data, test_data):
    ...
    print("Accuracy: %f" % accuracy_from_conf(conf))
    print(conf)
    print(report)
...
def plot_roc_curve(roc_metric):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig("roc_curve_threshold.pdf")
...
def plot_pr_curve(pr_metric):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig("pr_curve_threshold.pdf")
...
if __name__ == "__main__":
    ...
    # presence method
    train_data = get_train_data(100, False)
    test_data = get_test_data(train_data['vocab'], False)
